...
def globavg_severalvars_timeseries_landonly(testdir,varname1,factor1,varname2,factor2,varname3,factor3,height3,runmin,runmax,squareland):
    if squareland: 
...
# And finally we get to plot things...
    ...
    # plt.plot(months,timeseries1,label=varname1)
    # plt.plot(months,timeseries2,label=varname2)
    # plt.plot(months,timeseries3,label=varname3+' at pfull = '+str(nc.variables['pfull'][height3]))
    # plt.title('globavg '+varname1+', '+varname2+' and '+varname3)
    # plt.xlabel('Month #')
    # plt.ylabel('Global average')
    # plt.grid(b=True)
    # plt.legend()
...
def globavg_var_timeseries_total_and_land(testdir,varname,runmin,runmax,factor,squareland):
    ...
    months=np.linspace(runmin,(runmax-1),timeseries.size)
    plt.plot(months,timeseries,label='total')
    plt.plot(months,timeseries_land,label='land only')
    plt.title('globavg '+varname+' total and land only')
    plt.xlabel('Month #')
    plt.ylabel('Global average')
    plt.legend()
    plt.grid(b=True)
...
def squareland_inputfile(filename,varname):
	...
	array=xr.DataArray(nc.variables[varname][i,:,:])
...
# ys = [sq_minlat,sq_minlat,sq_maxlat,sq_maxlat,sq_minlat]
	...
	m.drawgreatcircle(square_lons.min(), square_lats.min(), square_lons.min(), square_lats.max()+dlats+dlats, del_s=100., color='b')
	m.drawgreatcircle(square_lons.max(), square_lats.min(), square_lons.max(), square_lats.max()+dlats+dlats, del_s=100., color='b')
	...
	lats = [square_lats.max()+dlats+dlats,square_lats.max()+dlats+dlats] 
	lons = [square_lons.min(),square_lons.max()] 
	x, y = m(lons, lats) 
	m.plot(x, y, color='b') 
	plt.title(varname)	
